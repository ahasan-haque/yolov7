...
from utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams, LoadGroundTruths
...
def run(
    ...
    if webcam:
        ...
        ground_truth = LoadGroundTruths(gt, img_size=imgsz, stride=stride, auto=pt)
        bs = 1  # batch_size
    ...
    for (path, im, im0s, vid_cap, s), (gt_path, gt_im, gt_im0s, gt_vid_cap, gt_s) in zip(dataset, ground_truth):
        #print(gt_path, gt_im.shape, gt_im0s.shape, gt_vid_cap, gt_s)
        with dt[0]:
    ...
    print(f"mean IOU: {total_intersection/total_union}")
    # Print results
